...
st.set_page_config(page_title="Painel Transportes - Fujicom", page_icon="­ƒÜÜ", layout="wide", initial_sidebar_state="expanded")
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Entrar", type="primary", use_container_width=True):
                ...
                if email_ok and pw_ok:
                    ...
                    st.error("Email n├úo autorizado")
                else:
...
st.sidebar.markdown(f"­ƒæñ {st.session_state.auth_email}", unsafe_allow_html=True)
if st.sidebar.button("­ƒÜ¬ Sair", type="primary"):
    st.session_state.auth_email = None
    st.rerun()
st.sidebar.markdown("---")
...
if CLOUD_MODE:
    st.sidebar.success("Ôÿü´©Å Modo nuvem")
else:
    st.sidebar.info("­ƒÆ╗ Modo local")

if not CLOUD_MODE:
    st.sidebar.markdown("### ­ƒöæ Conex├úo Google Sheets")
...
def status_color(s):
    ...
    if "entreg" in s:
        return "#27ae60"
    if "tr├ónsito" in s or "transito" in s or "colet" in s or "origem" in s:
        return "#2980b9"
...
# ÔöÇÔöÇÔöÇ LOAD / CONNECT ÔöÇÔöÇÔöÇ
creds_json = None
sheet_url = None
...
if CLOUD_MODE:
    ...
    if "gs_creds" in st.session_state and "gs_url" in st.session_state:
...
if not creds_json or not sheet_url:
    st.info("­ƒæê Conecte-se ao Google Sheets para carregar os par├ómetros")
    st.stop()

# ÔöÇÔöÇÔöÇ GEBEX CONFIG ÔöÇÔöÇÔöÇ
GBEX_FILE_DEFAULT = os.path.join(os.path.dirname(__file__), "..", "Fujicom", "GBEX", "OCOREN_FUJICOM.TXT")
gebex_file_path = st.sidebar.text_input("­ƒôü Arquivo OCOREN GBEX", value=st.session_state.get("gebex_path", GBEX_FILE_DEFAULT), key="gebex_path_input")

# ÔöÇÔöÇÔöÇ LOAD PARAMETERS ÔöÇÔöÇÔöÇ
try:
    sheet = gsheet_connect(creds_json, sheet_url)
    st.sidebar.success("Ô£à Conectado ao Google Sheets")
...
try:
    ...
    ws_params.append_row(["usuario", "2323120000236_PRD", "Braspress", "api", "Usu├írio API"])
    ...
    ws_params.append_row(["nfs", "", "Braspress", "tracking", "NFs para rastrear (separadas por v├¡rgula)"])
    ws_params.append_row(["pedidos", "", "Braspress", "tracking", "N┬║ Pedidos para rastrear (separados por v├¡rgula)"])
    df_params = df_from_ws(ws_params)
...
# ÔöÇÔöÇÔöÇ LOAD BRASPRESS DATA ÔöÇÔöÇÔöÇ
...
# ÔöÇÔöÇÔöÇ LOAD GEBEX DATA ÔöÇÔöÇÔöÇ
...
gb_transito = len(df_gb[df_gb["status"].str.lower().str.contains("tr├ónsito|transito", na=False)]) if not df_gb.empty else 0

# ÔöÇÔöÇÔöÇ MAIN HEADER ÔöÇÔöÇÔöÇ
...
br_transito = len(df_br[df_br["status"].str.lower().str.contains("tr├ónsito|transito|colet", na=False)]) if not df_br.empty and "status" in df_br.columns else 0
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem;">
    <div>
        <h1 style="margin:0;">­ƒÜÜ Painel Transportes - Fujicom</h1>
        <p style="color:#6c757d; margin:0;">{total_conhecimentos} registros ÔÇó {entregues} entregues ÔÇó {em_transito} em tr├ónsito</p>
    </div>
...
with k1:
    st.markdown(f"""<div class="kpi-card" style="border-left-color: #27ae60;"><div class="label">­ƒôª Total Registros</div><div class="value">{total_conhecimentos}</div><div class="sub">Braspress {len(df_br)} + GEBEX {len(df_gb)}</div></div>""", unsafe_allow_html=True)
...
with k2:
    st.markdown(f"""<div class="kpi-card" style="border-left-color: #2980b9;"><div class="label">­ƒÜø Em Tr├ónsito</div><div class="value">{em_transito}</div><div class="sub">{pct_transito:.1f}%</div></div>""", unsafe_allow_html=True)
with k3:
    st.markdown(f"""<div class="kpi-card" style="border-left-color: #27ae60;"><div class="label">Ô£à Entregues</div><div class="value">{entregues}</div><div class="sub">{pct_entregues:.1f}%</div></div>""", unsafe_allow_html=True)
with k4:
    pct_pendentes = pendentes / total_conhecimentos * 100 if total_conhecimentos > 0 else 0
    st.markdown(f"""<div class="kpi-card" style="border-left-color: #f39c12;"><div class="label">ÔÅ│ Pendentes / Atrasados</div><div class="value">{pendentes}</div><div class="sub">{pct_pendentes:.1f}%</div></div>""", unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ÔöÇÔöÇÔöÇ TABS ÔöÇÔöÇÔöÇ
tabs = st.tabs(["­ƒôê Vis├úo Geral", "­ƒÜø Braspress", "­ƒôª GEBEX", "ÔÜÖ´©Å Par├ómetros", "­ƒôï Dados Export├íveis"])
tab_geral, tab_br, tab_gb, tab_params, tab_export = tabs

# ÔöÇÔöÇÔöÇ TAB: VIS├âO GERAL ÔöÇÔöÇÔöÇ
with tab_geral:
    # --- Search Area ---
    st.markdown('<div class="card">', unsafe_allow_html=True)
    sc1, sc2 = st.columns([1, 1])
    with sc1:
        st.markdown('<h3 style="color:#e8edf5;">­ƒöÄ Buscar Nota Fiscal</h3>', unsafe_allow_html=True)
        nf_busca = st.text_input("Digite o n├║mero da NF", placeholder="Ex: 21070", key="nf_busca_geral", label_visibility="collapsed")
    ...
    with sc2:
        st.markdown('<h3 style="color:#e8edf5;">­ƒæÑ Buscar Cliente</h3>', unsafe_allow_html=True)
        ...
        with col_cli_periodo:
            period_option = st.selectbox("Per├¡odo", ["├Ültimos 30 dias", "├Ültimos 90 dias", "├Ültimos 6 meses", "Todo per├¡odo"], key="periodo_cli")
    ...
    if busca_nf_click and nf_busca.strip():
        ...
        if not match.empty:
            ...
            st.markdown(f'<h3 style="color:#e8edf5;">­ƒôª Panorama da NF {nf_clean}</h3>', unsafe_allow_html=True)
            p1, p2, p3, p4 = st.columns(4)
            with p1:
                emissao = str(row.get("emissao", ""))[:10] if pd.notna(row.get("emissao")) else "-"
                st.markdown(f"""<div class="kpi-card" style="border-left-color:#2980b9;"><div class="label">­ƒôà Data Emiss├úo</div><div class="value" style="font-size:1.1rem;">{emissao}</div></div>""", unsafe_allow_html=True)
            with p2:
                prev = str(row.get("previsaoEntrega", ""))[:10] if pd.notna(row.get("previsaoEntrega")) else "-"
                st.markdown(f"""<div class="kpi-card" style="border-left-color:#f39c12;"><div class="label">­ƒôà Previs├úo Entrega</div><div class="value" style="font-size:1.1rem;">{prev}</div></div>""", unsafe_allow_html=True)
            with p3:
                sts = str(row.get("status", "-"))
                cor = "#27ae60" if "entreg" in sts.lower() else "#2980b9" if "tr├ónsito" in sts.lower() or "colet" in sts.lower() else "#e74c3c"
                st.markdown(f"""<div class="kpi-card" style="border-left-color:{cor};"><div class="label">­ƒôî Status</div><div class="value" style="font-size:1.1rem;">{sts}</div></div>""", unsafe_allow_html=True)
            with p4:
                st.markdown(f"""<div class="kpi-card" style="border-left-color:#8e44ad;"><div class="label">­ƒÜø Transportadora</div><div class="value" style="font-size:1.1rem;">{transp}</div></div>""", unsafe_allow_html=True)

            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f"**Destinat├írio:** {row.get('destinatario', '-')}")
                st.markdown(f"**Remetente:** {row.get('remetente', '-')}")
                st.markdown(f"**Origem:** {row.get('origem', '-')} ÔåÆ **Destino:** {row.get('cidade', '-')}/{row.get('uf', '-')}")
            with c2:
                ult_oco = str(row.get("ultimaOcorrencia", "-"))
                dt_oco = str(row.get("dataOcorrencia", ""))[:10] if pd.notna(row.get("dataOcorrencia")) else "-"
                st.markdown(f"**├Ültima Ocorr├¬ncia:** {ult_oco}")
            ...
            st.warning(f"NF {nf_clean} n├úo encontrada em nenhuma transportadora.")
    ...
    if busca_cli_click and cli_busca.strip():
        ...
        if not cli_results.empty:
            # Apply period filter
            if period_option == "├Ültimos 30 dias":
            ...
            elif period_option == "├Ültimos 90 dias":
                cutoff = datetime.now() - timedelta(days=90)
                if "emissao" in cli_results.columns:
                    cli_results["_dt"] = pd.to_datetime(cli_results["emissao"], errors="coerce", dayfirst=True)
                    cli_results = cli_results[cli_results["_dt"] >= cutoff]
            elif period_option == "├Ültimos 6 meses":
            ...
            if not cli_results.empty:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.markdown(f'<h3 style="color:#e8edf5;">­ƒôï Notas de {cli_clean} ({len(cli_results)} registros)</h3>', unsafe_allow_html=True)
                ...
                df_cli_show.columns = [c.replace("_transportadora","Transp.").replace("nf_numero","NF").replace("numero","CTE").replace("emissao","Emiss├úo").replace("previsaoEntrega","Prev.Entrega").replace("status","Status").replace("cidade","Cidade").replace("uf","UF").replace("destinatario","Cliente").replace("valorMercantil","Valor") for c in existing_display]
                ...
                st.info(f"Nenhum registro de {cli_clean} no per├¡odo selecionado.")
        else:
            st.warning(f"Cliente '{cli_clean}' n├úo encontrado.")

    # --- Carrier Overview ---
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<h3 style="color:#e8edf5;">Vis├úo Geral por Transportadora</h3>', unsafe_allow_html=True)
    ...
    with cc1:
        ...
        <div class="block-config">
            ...
            <p class="param">Em tr├ónsito: <span>{}</span></p>
            <p class="param">├Ültima consulta: <span>{}</span></p>
        </div>
        """.format(
            len(df_br),
            br_entregues,
    ...
    with cc2:
        ...
        <div class="block-config">
            ...
            <p class="param">Entregues: <span>{}</span></p>
            <p class="param">Em tr├ónsito: <span>{}</span></p>
            <p class="param">├Ültima atualiza├º├úo: <span>{}</span></p>
        </div>
        """.format(
            len(df_gb),
            gb_entregues,
    ...
    with cc3:
        ...
        <div class="block-config">
            <h4>­ƒôª DHL</h4>
            <p class="param"><span style="color:#f39c12;">ÔÅ│ Configura├º├úo pendente</span></p>
...
# ÔöÇÔöÇÔöÇ TAB: BRASPRESS ÔöÇÔöÇÔöÇ
with tab_br:
    from braspress_api import BraspressAPI, flatten_conhecimentos

    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("­ƒÜø Braspress - Rastreamento")

    ac1, ac2, ac3 = st.columns([1, 1, 1])
    with ac1:
        atualizar = st.button("­ƒöä Atualizar Dados da API", type="primary", use_container_width=True)
    with ac2:
    ...
    if atualizar:
        if not BR_NFS and not BR_PEDIDOS:
            st.warning("ÔÜá´©Å Nenhuma NF ou Pedido configurado para rastrear. Adicione na aba Par├ómetros ou acima.")
        else:
            ...
            if todos:
                ...
                df_br = df_novo
                st.success(f"Ô£à {len(todos)} conhecimentos atualizados!")
                st.rerun()
            else:
    ...
    if not df_br.empty:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("­ƒôï Conhecimentos Braspress")

        # Auto-filter when navigated from Vis├úo Geral
        ...
        with f3:
            ...
            search = st.text_input("­ƒöÄ Buscar", placeholder="NF, n┬║ conhecimento...", key="br_search", value=default_search)
        ...
        e1.download_button("­ƒôÑ CSV", csv, f"braspress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine="openpyxl") as w:
            df_show.to_excel(w, index=False, sheet_name="Braspress")
        e2.download_button("­ƒôÑ Excel", output.getvalue(), f"braspress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        e3.download_button("­ƒôÑ JSON", df_show.to_json(orient="records", force_ascii=False, indent=2).encode("utf-8-sig"), f"braspress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "application/json", use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)

        # KPIs Braspress
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("­ƒôè M├®tricas Braspress")
        bk1, bk2, bk3, bk4 = st.columns(4)
        with bk1:
            total_frete = pd.to_numeric(df_br["totalFrete"], errors="coerce").sum() if "totalFrete" in df_br.columns else 0
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #8e44ad;"><div class="label">­ƒÆ░ Total Frete</div><div class="value">R$ {total_frete:,.2f}</div></div>""", unsafe_allow_html=True)
        with bk2:
            total_valor = pd.to_numeric(df_br["valorMercantil"], errors="coerce").sum() if "valorMercantil" in df_br.columns else 0
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #2980b9;"><div class="label">­ƒôª Valor Mercantil</div><div class="value">R$ {total_valor:,.2f}</div></div>""", unsafe_allow_html=True)
        with bk3:
            total_peso = pd.to_numeric(df_br["peso"], errors="coerce").sum() if "peso" in df_br.columns else 0
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #27ae60;"><div class="label">ÔÜû´©Å Peso Total</div><div class="value">{total_peso:,.0f} kg</div></div>""", unsafe_allow_html=True)
        with bk4:
            total_volumes = pd.to_numeric(df_br["volumes"], errors="coerce").sum() if "volumes" in df_br.columns else 0
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #f39c12;"><div class="label">­ƒôè Total Volumes</div><div class="value">{total_volumes:,.0f}</div></div>""", unsafe_allow_html=True)
        ...
        with cc1:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.subheader("Evolu├º├úo por Status")
        ...
        with cc2:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.subheader("Top Destinat├írios")
        ...
        st.info("­ƒô¡ Nenhum dado de rastreamento dispon├¡vel. Clique em 'Atualizar Dados da API' ou configure NFs/Pedidos.")

# ÔöÇÔöÇÔöÇ TAB: GEBEX ÔöÇÔöÇÔöÇ
with tab_gb:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("­ƒôª GEBEX - Ocorr├¬ncias")

    if not df_gb.empty:
        col_gb1, col_gb2 = st.columns(2)
        with col_gb1:
            if st.button("­ƒöä Recarregar do arquivo", type="secondary", key="gb_reload"):
                st.rerun()
        with col_gb2:
            st.markdown(f"**Arquivo:** `{gebex_file_path}`")

        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("­ƒôï NFs Rastreadas")
        search_gb = st.text_input("­ƒöÄ Buscar", placeholder="NF, status...", key="gb_search")
        ...
        # Detailed timeline
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("ÔÅ│ Timeline da NF")
        nf_timeline = st.text_input("Digite a NF para ver o hist├│rico", placeholder="Ex: 47696", key="gb_timeline_input")
        if nf_timeline:
            ...
            if ocor_filtradas:
                ...
                for o in ocor_filtradas:
                    ...
                    <div style="background:#16233a; border-left:3px solid {cor}; padding:0.5rem 1rem; margin:0.3rem 0; border-radius:6px;">
                        <strong>{o.get('data_ocorrencia', '')}</strong> ÔÇö {o.get('descricao_ocorrencia', '')}
                        <span style="color:#8899b8; font-size:0.8rem;"> (c├│d. {o.get('codigo_ocorrencia', '')})</span>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.info(f"NF {nf_clean} n├úo encontrada no arquivo de ocorr├¬ncias.")
        st.markdown("</div>", unsafe_allow_html=True)

        # KPIs
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("­ƒôè M├®tricas GEBEX")
        gk1, gk2, gk3, gk4 = st.columns(4)
        with gk1:
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #27ae60;"><div class="label">­ƒôª Total NFs</div><div class="value">{len(df_gb)}</div></div>""", unsafe_allow_html=True)
        with gk2:
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #2980b9;"><div class="label">­ƒÜø Em Tr├ónsito</div><div class="value">{gb_transito}</div></div>""", unsafe_allow_html=True)
        with gk3:
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #27ae60;"><div class="label">Ô£à Entregues</div><div class="value">{gb_entregues}</div></div>""", unsafe_allow_html=True)
        pend_gb = len(df_gb) - gb_entregues - gb_transito
        with gk4:
            st.markdown(f"""<div class="kpi-card" style="border-left-color: #f39c12;"><div class="label">ÔÅ│ Pendentes</div><div class="value">{pend_gb}</div></div>""", unsafe_allow_html=True)
        ...
        st.info(f"­ƒô¡ Nenhum dado GEBEX encontrado em `{gebex_file_path}`. Verifique se o arquivo OCOREN existe.")
    st.markdown("</div>", unsafe_allow_html=True)

# ÔöÇÔöÇÔöÇ TAB: PAR├éMETROS ÔöÇÔöÇÔöÇ
with tab_params:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("ÔÜÖ´©Å Par├ómetros do Sistema")

    st.markdown("""
    <p style="color:#8899b8; font-size:0.85rem;">
        Esta aba reflete os par├ómetros da planilha. Edite diretamente na planilha Google Sheets e clique em "Recarregar".
    </p>
    """, unsafe_allow_html=True)

    if st.button("­ƒöä Recarregar Par├ómetros", type="secondary"):
        st.rerun()
    ...
    with bcols[1]:
        st.text_input("Usu├írio", value=BR_USUARIO, key="p_user", disabled=True)
    with bcols[2]:
    ...
    gb_status = "Ô£à Ativo (arquivo local)" if gb_file_exists else "ÔÅ│ Aguardando arquivo OCOREN"
    ...
    with gcols[1]:
        st.text_input("Usu├írio FTP", value="", key="p_ftp_user", disabled=True, placeholder="Em breve")
    with gcols[2]:
    ...
    <div class="block-config">
        <h4>­ƒôª DHL</h4>
        <p class="param">Status: <span style="color:#f39c12;">ÔÅ│ A implementar</span></p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("</div>", unsafe_allow_html=True)

# ÔöÇÔöÇÔöÇ TAB: DADOS EXPORT├üVEIS ÔöÇÔöÇÔöÇ
with tab_export:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("­ƒôï Dados Export├íveis")

    ed_tabs = st.tabs(["­ƒôï Braspress", "­ƒôª GEBEX", "­ƒôª DHL"])
    with ed_tabs[0]:
        if not df_br.empty:
            st.caption(f"{len(df_br)} registros")
            search_ed = st.text_input("­ƒöÄ Buscar", placeholder="Digite para filtrar...", key="ed_search_br")
            df_ed = df_br.copy()
            if search_ed:
            ...
            e1.download_button("­ƒôÑ CSV", csv_ed, f"braspress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
            output_ed = io.BytesIO()
            with pd.ExcelWriter(output_ed, engine="openpyxl") as w:
                df_ed.to_excel(w, index=False, sheet_name="Braspress")
            e2.download_button("­ƒôÑ Excel", output_ed.getvalue(), f"braspress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            e3.download_button("­ƒôÑ JSON", df_ed.to_json(orient="records", force_ascii=False, indent=2).encode("utf-8-sig"), f"braspress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "application/json", use_container_width=True)
        else:
            st.info("Nenhum dado dispon├¡vel.")
    with ed_tabs[1]:
        if not df_gb.empty:
            st.caption(f"{len(df_gb)} registros")
            search_ed_gb = st.text_input("­ƒöÄ Buscar", placeholder="Digite para filtrar...", key="ed_search_gb")
            df_ed_gb = df_gb.copy()
            ...
            eg1.download_button("­ƒôÑ CSV", csv_ed_gb, f"gebex_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
            ...
            eg2.download_button("­ƒôÑ Excel", output_ed_gb.getvalue(), f"gebex_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            eg3.download_button("­ƒôÑ JSON", df_ed_gb.to_json(orient="records", force_ascii=False, indent=2).encode("utf-8-sig"), f"gebex_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "application/json", use_container_width=True)
        else:
            st.info("­ƒôª GEBEX - Aguardando dados do arquivo OCOREN.")
    with ed_tabs[2]:
        st.info("­ƒôª DHL - Aguardando configura├º├úo.")
    st.markdown("</div>", unsafe_allow_html=True)
